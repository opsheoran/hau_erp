...
def employee_foreign_visit_details():
    ...
    lookups = {
        'sponsors': DB.fetch_all("SELECT PK_FSponsor_Id as id, FName as name FROM SAL_FundSponsor_Mst ORDER BY ISNULL(DisplayOrder, 999999), FName")
    }
...
def sar_admin_transaction():
    ...
    sar_data = SARModel.get_sar_lists(category, fin_id=session_id, dept_id=dept_id)
    
    # Filter by session and department if provided
    # (Note: model currently doesn't filter by these, but we could add if needed)
    
    lookups = {
        'sessions': DB.fetch_all("SELECT pk_finid as id, CAST(YEAR(date1) AS varchar) + '-' + CAST(YEAR(date2) AS varchar) as name FROM SAL_Financial_Year ORDER BY date1 DESC"),
...
def manage_master(key):
    ...
    pagination, sql_limit = get_pagination(cfg['table'], page, order_by=f"ORDER BY {cfg['name']}")
    
    data = DB.fetch_all(f"SELECT * FROM {cfg['table']} {sql_limit}")
...
def city_master():
    ...
    page = to_int(request.args.get('page', 1)); pagination, sql_limit = get_pagination("SAL_City_Mst C", page, order_by="ORDER BY C.cityname")
...
def relation_master():
    ...
    page = to_int(request.args.get('page', 1)); pagination, sql_limit = get_pagination("Relation_MST", page, order_by="ORDER BY Relation_name")
    relations = DB.fetch_all(f"SELECT Pk_Relid as id, Relation_name as name, Remarks FROM Relation_MST {sql_limit}")
...
def category_master():
    ...
    page = to_int(request.args.get('page', 1)); pagination, sql_limit = get_pagination("SAL_Category_Mst", page, order_by="ORDER BY category")
    categories = DB.fetch_all(f"SELECT pk_catid as id, category as name FROM SAL_Category_Mst {sql_limit}")
...
def department_master():
    ...
    page = to_int(request.args.get('page', 1)); pagination, sql_limit = get_pagination("Department_Mst D", page, where=where, params=params, order_by="ORDER BY D.description")
...
def class_master():
    ...
    page = to_int(request.args.get('page', 1)); pagination, sql_limit = get_pagination("SAL_Class_Mst C", page, order_by="ORDER BY C.classname")
...
def office_type_master():
    ...
    types = DB.fetch_all(f"SELECT OfficeTypeID as id, code, OfficeName as name FROM OfficeTypeMaster {sql_limit}")
...
def employee_master():
    ...
    if request.method == 'POST':
        ...
        if action in ('SEARCH', 'VIEW'):
            args = {
                ...
                's_manual_emp_code': sval('s_manual_emp_code'),
                's_emp_name': sval('s_emp_name'),
                's_ctrl_id': sval('s_ctrl_id'),
                's_loc_id': sval('s_loc_id'),
                's_ddo_id': sval('s_ddo_id'),
                's_desg_id': sval('s_desg_id'),
                's_dept_id': sval('s_dept_id'),
                's_nature_id': sval('s_nature_id'),
                's_section_id': sval('s_section_id'),
                's_fund_id': sval('s_fund_id'),
                's_city_id': sval('s_city_id'),
                's_scheme_group': sval('s_scheme_group'),
                's_status': sval('s_status'),
                's_scheme_id': sval('s_scheme_id'),
                's_sort_by': sval('s_sort_by'),
                's_med_type': sval('s_med_type'),
                'page': 1
            }
            ...
            try:
                ...
                DB.execute('''
                    ...
                    SET manualempcode = ?,
                        ...
                        fk_fundid = ?,
                        fk_anc = ?,
                        fk_SubGroupId = COALESCE(?, fk_SubGroupId),
                        fk_cfundid = COALESCE(?, fk_cfundid),
                        fk_cSubGroupId = COALESCE(?, fk_cSubGroupId),
                        fk_anc_child = COALESCE(?, fk_anc_child),
                        leftdate = TRY_CONVERT(date, NULLIF(?, ''), 23),
                        employeeleftstatus = ?,
                        leftreason = ?,
                        leftremarks = ?,
                        reportingto = ?,
                        incdate = TRY_CONVERT(date, NULLIF(?, ''), 23),
                        pfileno = ?,
                        mbfno = ?,
                        pgteachercode = ?,
                        sipremno = ?,
                ...
                ''', [
                    ...
                    sid('scheme_group_id'), sid('fund_id_2'), sid('scheme_group_id_2'), sid('scheme_id_2'),
                ...
                if other_exists:
                    DB.execute('''
                        ...
                        SET gender = ?,
                            fk_catid = ?,
                            fk_religionid = ?,
                            dateofbirth = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateofappointment = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateofjoining = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateofretirement = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateoflastappointment = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateoflastjoining = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            OrderNo = ?,
                            fk_quarterid = ?,
                            QuarterEffecDate = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            AppointmentTime = ?,
                            remarks = ?,
                            fk_updUserID = ?,
                            fk_updDateID = GETDATE()
                ...
                # Update gpf_employee_details
                gpf_exists = int(DB.fetch_scalar('SELECT COUNT(*) FROM gpf_employee_details WHERE fk_empid = ?', [emp_id]) or 0) > 0
                pf_type_input = request.form.get('pf_type')
                pf_no_input = request.form.get('pf_no')
                
                if gpf_exists:
                    DB.execute('UPDATE gpf_employee_details SET PFType = ?, gpfno = ?, fk_updDateID = GETDATE() WHERE fk_empid = ?', [pf_type_input, pf_no_input, emp_id])
                else:
                    DB.execute('INSERT INTO gpf_employee_details (fk_empid, PFType, gpfno, fk_insDateID) VALUES (?, ?, ?, GETDATE())', [emp_id, pf_type_input, pf_no_input])
                        ...
                        if exists_hd:
                            DB.execute('''
                                ...
                                SET amount = TRY_CONVERT(decimal(18,2), NULLIF(?, '')),
                                    effectdate = TRY_CONVERT(date, NULLIF(?, ''), 23),
                                    ismanual = ?,
                                    fk_updUserID = ?,
                                    fk_updDateID = GETDATE()
                            ...
                            DB.execute('''
                                ...
                                VALUES (?, ?, ?, TRY_CONVERT(decimal(18,2), NULLIF(?, '')), TRY_CONVERT(date, NULLIF(?, ''), 23), ?, ?, GETDATE())
    ...
    filters = {
        ...
        'manual_empcode': (request.args.get('s_manual_emp_code') or request.form.get('s_manual_emp_code') or '').strip(),
        ...
        'loc_id': (request.args.get('s_loc_id') or request.form.get('s_loc_id') or '').strip(),
        'ddo_id': (request.args.get('s_ddo_id') or request.form.get('s_ddo_id') or '').strip(),
        'nature_id': (request.args.get('s_nature_id') or request.form.get('s_nature_id') or '').strip(),
        'section_id': (request.args.get('s_section_id') or request.form.get('s_section_id') or '').strip(),
        'fund_id': (request.args.get('s_fund_id') or request.form.get('s_fund_id') or '').strip(),
        'city_id': (request.args.get('s_city_id') or request.form.get('s_city_id') or '').strip(),
        'scheme_group': (request.args.get('s_scheme_group') or request.form.get('s_scheme_group') or '').strip(),
        'status': (request.args.get('s_status') or request.form.get('s_status') or 'A').strip(),
        'scheme_id': (request.args.get('s_scheme_id') or request.form.get('s_scheme_id') or '').strip(),
        'sort_by': (request.args.get('s_sort_by') or request.form.get('s_sort_by') or 'manualempcode').strip(),
        'med_type': (request.args.get('s_med_type') or request.form.get('s_med_type') or '').strip()
    }

    page = to_int(request.args.get('page', 1))
    edit_id = request.args.get('edit_id')

    total = to_int(EmployeeModel.count_employees_detailed(filters) or 0)
    pagination = {'page': page, 'per_page': 10, 'total': total, 'total_pages': max(1, (total + 9) // 10)}
    ...
    if edit_id:
        ...
        if edit_data:
            ...
            # Special logic for GPF Advance installment
            if edit_data.get('gpf_adv_inst'):
                for h in deduction_heads:
                    if int(h.get('head_id') or 0) == 27: # GPF Advance head_id
                        h['amount'] = edit_data['gpf_adv_inst']
            
            # Special logic for Income Tax
            if edit_data.get('income_tax'):
                for h in deduction_heads:
                    if int(h.get('head_id') or 0) == 130: # Income Tax head_id
                        h['amount'] = edit_data['income_tax']
            ...
            income_tax = edit_data.get('income_tax') or 0.00
...
def employee_master_scheme_wise():
    ...
    if request.method == 'POST':
        ...
        if action == 'RESET':
            return redirect(url_for('establishment.employee_master_scheme_wise'))
        ...
        if action in ('SEARCH', 'VIEW'):
            ...
            args = {k: v for k, v in args.items() if v}
            return redirect(url_for('establishment.employee_master_scheme_wise', **args))
        ...
        if action in ('SAVE', 'UPDATE'):
            ...
            if (not is_new) and not perm.get('AllowUpdate'):
                flash('You do not have permission to update.', 'danger')
                return redirect(url_for('establishment.employee_master_scheme_wise', edit_id=emp_id))
    ...
    if edit_id:
        ...
        if edit_data:
            ...
            if edit_data.get('gpf_adv_inst'):
                for h in deduction_heads:
                    if int(h.get('head_id') or 0) == 27:
                        h['amount'] = edit_data['gpf_adv_inst']

            if edit_data.get('income_tax'):
                for h in deduction_heads:
                    if int(h.get('head_id') or 0) == 130:
                        h['amount'] = edit_data['income_tax']
    ...
    return render_template(
        'establishment/employee_master_scheme_wise.html',
        employees=employees,
        lookups=lookups,
        edit_data=edit_data,
        perm=perm,
...
def manage_standard_master(key, title, table, pk, name_col, order_col=None):
    ...
    pagination, sql_limit = get_pagination(table, page, order_by=f"ORDER BY {order_by}")
    
    data = DB.fetch_all(f"SELECT {pk} as id, {name_col} as name" + (f", {order_col} as order_no" if order_col else "") + f" FROM {table} {sql_limit}")
                ...
                if exists:
                    DB.execute('''
                        ...
                        SET fk_quarterid = ?, joininglocation = ?, fk_desgid_j = ?, fk_religionid = ?, gender = ?, fk_catid = ?,
                            dateofappointment = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateofjoining = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateOfConfirmation = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateofretirement = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateoflastappointment = TRY_CONVERT(date, NULLIF(?, ''), 23),
                            dateoflastjoining = TRY_CONVERT(date, NULLIF(?, ''), 23),
