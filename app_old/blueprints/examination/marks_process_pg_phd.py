...
@examination_bp.route('/marks_process_pg_phd', methods=['GET', 'POST'])
@permission_required('Marks Process for PG')
def marks_process_pg_phd():
    ...
    filters = {
        ...
        'exam_type': request.args.get('exam_type') or request.form.get('exam_type', '1') # 1: Regular/Back
    }
        ...
        if action == 'GET_STUDENTS':
            ...
            if not student_ids:
                ...
                for sid in student_ids:
                    ...
                    ''', [sid, session_id, filters['class_id']])
                    ...
                    for m in marks:
                        ...
                        if aid not in marks_dict: marks_dict[aid] = {}
                    ...
                    for c in courses:
                        ...
                        total_obt = ith['val'] + ipr['val']
                        total_max = ith['max'] + ipr['max']
                        ...
                        if cld_exists:
                            ...
                            ''', [gp, cp, 1 if is_passed else 0, th_pf, pr_pf, 
                                  ith['val'], ith['max'], ipr['val'], ipr['max'], aid])
                        else:
                            DB.execute('''
                ...
                flash(f"Marks successfully processed for {count} student(s).", "success")
            return redirect(url_for('examination.marks_process_pg_phd', **filters))

    return render_template('examination/marks_process_pg_phd.html', lookups=lookups, filters=filters, students=students)


@examination_bp.route('/api/get_student_courses_pg_phd')
def get_student_courses_pg_phd():
    ...
    if alloc_ids:
        ...
        for m in marks:
            ...
            if alloc_id not in marks_dict: marks_dict[alloc_id] = {}
            exam_id = m['pk_examid']
        ...
        ''', alloc_ids)
        for cld in clds: cld_dict[cld['fk_stucourseallocid']] = cld
    ...
    for c in courses:
        ...
        def format_mark(m_obj):
            if m_obj and m_obj.get('absent'): return "A"
            if m_obj and m_obj['val'] is not None:
                val = float(m_obj['val'])
                max_val = float(m_obj['max'] or 0)
                if val == 0 and max_val == 0: return "0.000/0"
...
@examination_bp.route('/api/get_college_pg_phd_degrees')
def get_college_pg_phd_degrees():
    college_id = request.args.get('college_id')
    if not college_id: return jsonify([])
    query = '''
        ...
        WHERE M.fk_collegeid = ? AND D.fk_degreetypeid IN (2, 4) AND D.degreename NOT LIKE '%MBA%' AND D.degreename NOT LIKE '%M.B.A%'
