...
@examination_bp.route('/deanpgs_marks_approval', methods=['GET', 'POST'])
            ...
            if course_data:
                ...
                for item in course_data:
                    ...
                    if len(parts) == 3:
                        ...
                        # Apply DeanPGS approval
                        DB.execute('''
                            UPDATE D
                            SET D.IsApprovedByDeanPGS = 1, D.feeddate = GETDATE()
                        ...
                        ''', [request.form.get('session_id'), request.form.get('degree_id'), sem_id, year_id, cid])
                        count += 1
                if count > 0:
                    flash(f'Marks approved successfully by DeanPGS for {count} course(s).', 'success')
                else:
                    flash('No valid courses selected for approval.', 'warning')
            else:
                flash('Please select at least one course to approve.', 'warning')
            
            return redirect(url_for('examination.deanpgs_marks_approval', **request.form.to_dict()))
