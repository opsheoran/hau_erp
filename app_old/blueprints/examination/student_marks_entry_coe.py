...
@examination_bp.route('/student_marks_entry_coe', methods=['GET', 'POST'])
@permission_required('Student Marks Entry(@COE)')
def student_marks_entry_coe():
    if request.method == 'POST':
        ...
        return redirect(url_for('examination.student_marks_entry_coe',
    ...
    lookups = {
        ...
        'degrees': DB.fetch_all('SELECT pk_degreeid as id, degreename as name FROM SMS_Degree_Mst WHERE fk_degreetypeid = 3 ORDER BY degreename'),
    ...
    return render_template('examination/student_marks_entry_coe.html', 
                           lookups=lookups, filters=filters)

@examination_bp.route('/api/get_exam_configs_for_marks')
def get_exam_configs_for_marks():
    degree_id = request.args.get('degree_id')
    session_id = request.args.get('session_id')
...
@examination_bp.route('/api/get_courses_for_marks_entry')
def get_courses_for_marks_entry():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
    ...
    if not all([college_id, session_id, degree_id, class_id, year_id, exam_config_id]):
        return jsonify({'error': 'Missing required parameters (College, Session, Degree, Class, Year, Exam Config).'})
...
@examination_bp.route('/api/generate_marks_report_internal')
def generate_marks_report_internal():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
    ...
    # 1. Get Exams mapped to this degree/session/course
    exams_query = """
    ...
    # 2. Get Students allocated to this course in this session/degree
    students_query = """
    ...
    students_query += " ORDER BY RND.encryptuniv, S.enrollmentno"
    students = DB.fetch_all(students_query, params)

    # 3. For each student, get their already entered marks
    ...
    course_info = {
        ...
        'dept_name': 'Dean Office',  # Default as per template, can be fetched if needed
...
@examination_bp.route('/api/get_students_for_marks_entry')
def get_students_for_marks_entry():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id') # Semester
    course_id = request.args.get('course_id')
    year_id = request.args.get('year_id')
