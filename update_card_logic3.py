...
new_logic = """
    ...
    # The student is in curr_session and student['fk_semesterid']
    offered_map = set()
    offered_query = '''
    ...
    for p in pow_courses:
        ...
        # If it's allocated, check if it has custom semester credits (like Master's Research taking 9 out of 30)
        # We fall back to the master credits if no custom partial credit is set.
        crhr_th = p['master_th']
        crhr_pr = p['master_pr']
        ...
        # If it's explicitly a backlog from a previous fail that they are retaking THIS session
        if c['isbacklog']:
