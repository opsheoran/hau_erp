...
@examination_bp.route('/student_marks_entry_pg_phd_summer', methods=['GET', 'POST'])
@permission_required('Student Marks Entry PG/PhD Summer')
def student_marks_entry_pg_phd_summer():
    if request.method == 'POST':
                                ...
                                if existing:
                                    DB.execute("""
                                        ...
                                        SET marks_obt = ?, maxmarks = ?, isabsentt = ?, 
                                            isStudentMarksLocked = ?, islocked = ?, IsSupplySummerMarksFeed = 1, fk_userid = ?, feeddate = GETDATE()
                                    ...
                                    DB.execute("""
                                        ...
                                        (fk_stucourseallocid, fk_dgexammapid, marks_obt, maxmarks, isabsentt, isStudentMarksLocked, islocked, IsSupplySummerMarksFeed, fk_userid, feeddate)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, GETDATE())
        ...
        return redirect(url_for('examination.student_marks_entry_pg_phd_summer',
    ...
    lookups = {
        ...
        'degrees': AcademicsModel.get_pg_phd_degrees(),
    ...
    return render_template('examination/student_marks_entry_pg_phd_summer.html', 
                           lookups=lookups, filters=filters)



@examination_bp.route('/api/get_courses_for_marks_entry_pg_phd_summer')
def get_courses_for_marks_entry_pg_phd_summer():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
    ...
    query = '''
        ...
        AND ISNULL(A.IsSummer, 0) = 1
        AND EXISTS (
            SELECT 1 FROM SMS_TCourseAlloc_Mst tcam
            INNER JOIN SMS_TCourseAlloc_Dtl tcad ON tcam.pk_tcourseallocid = tcad.fk_tcourseallocid
            WHERE tcam.fk_sessionid = A.fk_dgacasessionid
              AND tcam.fk_collegeid = S.fk_collegeid
              AND tcam.fk_degreeid = DC.fk_degreeid
              AND tcam.fk_exconfigid = ?
              AND tcad.fk_courseid = C.pk_courseid
              AND tcam.fk_employeeid = (SELECT top 1 fk_empId FROM UM_Users_Mst WHERE pk_userId = ?)
        )
    '''
    params = [session_id, degree_id, college_id, class_id, year_id, exam_config_id, user_id]
...
@examination_bp.route('/api/get_students_for_marks_entry_pg_phd_summer')
def get_students_for_marks_entry_pg_phd_summer():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
    ...
    students_query = '''
        ...
        WHERE A.fk_courseid = ? AND A.fk_dgacasessionid = ? AND DC.fk_degreeid = ? AND ISNULL(A.IsSummer, 0) = 1 
    ...
    for s in students:
        ...
        for m in marks:
            if m['fk_stucourseallocid'] == s['Pk_stucourseallocid']:
                ...
                if m['isStudentMarksLocked'] == 1 or m.get('islocked') == 1:
...
@examination_bp.route('/api/generate_marks_report_pg_phd_summer')
def generate_marks_report_pg_phd_summer():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
