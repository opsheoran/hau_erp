...
@examination_bp.route('/student_marks_entry_revised_pg_phd', methods=['GET', 'POST'])
@permission_required('Student Marks Entry(Revised PG/PHD)')
def student_marks_entry_revised_pg_phd():
    if request.method == 'POST':
                                ...
                                if existing:
                                    DB.execute("""
                                        ...
                                        SET marks_obt = ?, maxmarks = ?, isabsentt = ?, 
                                            isStudentMarksLocked = ?, islocked = ?, IsRevisedMarksFeedPGPHD = 1, fk_userid = ?, feeddate = GETDATE()
                                    ...
                                    DB.execute("""
                                        ...
                                        (fk_stucourseallocid, fk_dgexammapid, marks_obt, maxmarks, isabsentt, isStudentMarksLocked, islocked, IsRevisedMarksFeedPGPHD, fk_userid, feeddate)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, GETDATE())
        ...
        return redirect(url_for('examination.student_marks_entry_revised_pg_phd',
    ...
    return render_template('examination/student_marks_entry_revised_pg_phd.html', 
                           lookups=lookups, filters=filters)



@examination_bp.route('/api/get_courses_for_marks_entry_revised_pg_phd')
def get_courses_for_marks_entry_revised_pg_phd():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
...
@examination_bp.route('/api/get_students_for_marks_entry_revised_pg_phd')
def get_students_for_marks_entry_revised_pg_phd():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
    ...
    students_query = '''
        ...
        WHERE A.fk_courseid = ? AND A.fk_dgacasessionid = ? AND DC.fk_degreeid = ? AND ISNULL(A.IsRevised, 0) = 1 
...
@examination_bp.route('/api/generate_marks_report_revised_pg_phd')
def generate_marks_report_revised_pg_phd():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
