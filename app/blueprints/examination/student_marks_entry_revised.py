...
@examination_bp.route('/student_marks_entry_revised', methods=['GET', 'POST'])
@permission_required('Student Marks Entry(Revised)')
def student_marks_entry_revised():
    if request.method == 'POST':
                    ...
                    for alloc_id in alloc_ids:
                        for emap_id in exam_map_ids:
                            ...
                            if mark_val is not None and mark_val != '':
                                ...
                                if existing:
                                    # Update logic for revised marks (you could add an IsRevised flag if applicable)
                                    DB.execute("""
        ...
        return redirect(url_for('examination.student_marks_entry_revised',
    ...
    return render_template('examination/student_marks_entry_revised.html', 
                           lookups=lookups, filters=filters)

@examination_bp.route('/api/get_courses_for_revised')
def get_courses_for_revised():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
...
@examination_bp.route('/api/get_students_for_revised')
def get_students_for_revised():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
