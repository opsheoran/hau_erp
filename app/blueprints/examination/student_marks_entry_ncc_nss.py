...
@examination_bp.route('/student_marks_entry_ncc_nss', methods=['GET', 'POST'])
@permission_required('Student Grading (NCC/NSS)')
def student_marks_entry_ncc_nss():
    if request.method == 'POST':
                ...
                if alloc_ids_str:
                    ...
                    for alloc_id in alloc_ids:
                        for emap_id in exam_map_ids:
                            grade_key = f"grade_{alloc_id}_{emap_id}"
                            ...
                            if grade_val or is_absent:
                                ...
                                if existing:
                                    DB.execute("""
                                        ...
                                        SET fk_gradeid = ?, isabsentt = ?, 
                                            isStudentMarksLocked = ?, islocked = ?, fk_userid = ?, feeddate = GETDATE()
                                        WHERE Pk_Stumarksdtlid = ?
                                    """, [grade_val, 1 if is_absent else 0, 1 if is_submit else 0, 1 if is_submit else 0, user_id, existing['Pk_Stumarksdtlid']])
                                else:
                                    DB.execute("""
                                        INSERT INTO SMS_StuExamMarks_Dtl
                                        (fk_stucourseallocid, fk_dgexammapid, fk_gradeid, maxmarks, marks_obt, isabsentt, isStudentMarksLocked, islocked, fk_userid, feeddate)
                                        VALUES (?, ?, ?, 0, 0, ?, ?, ?, ?, GETDATE())
                                    """, [alloc_id, emap_id, grade_val, 1 if is_absent else 0, 1 if is_submit else 0, 1 if is_submit else 0, user_id])
        ...
        return redirect(url_for('examination.student_marks_entry_ncc_nss',
    ...
    return render_template('examination/student_marks_entry_ncc_nss.html', 
                           lookups=lookups, filters=filters)



@examination_bp.route('/api/get_courses_for_marks_entry_ncc_nss')
def get_courses_for_marks_entry_ncc_nss():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
    ...
    query = '''
        ...
        AND C.isgrade = 1
        AND (C.coursename LIKE '%NCC%' OR C.coursename LIKE '%NSS%' OR C.coursename LIKE '%National Cadet%' OR C.coursename LIKE '%National Service%')
    '''
...
@examination_bp.route('/api/get_students_for_marks_entry_ncc_nss')
def get_students_for_marks_entry_ncc_nss():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
    ...
    if alloc_ids:
        ...
        marks = DB.fetch_all(f'''
            SELECT fk_stucourseallocid, fk_dgexammapid, marks_obt, fk_gradeid, isabsentt, isStudentMarksLocked, islocked
        ...
        for m in marks:
            if m['fk_stucourseallocid'] == s['Pk_stucourseallocid']:
                s_data['marks'][str(m['fk_dgexammapid'])] = {
                    'val': m['fk_gradeid'],  # Send grade ID instead of marks
    ...
    # Fetch Grades
    grades = DB.fetch_all("SELECT pk_marksgradeid as id, marksgrade as name FROM SMS_MarksGrade_Mst")
...
@examination_bp.route('/api/generate_marks_report_ncc_nss')
def generate_marks_report_ncc_nss():
    college_id = request.args.get('college_id')
    session_id = request.args.get('session_id')
    degree_id = request.args.get('degree_id')
    class_id = request.args.get('class_id')
