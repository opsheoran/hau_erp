...
class LoanModel:
    ...
    @staticmethod
    def get_loan_types():
        return DB.fetch_all("SELECT pk_loanid as id, loanname as name FROM SAL_Loan_Mst ORDER BY loanname")
...
class EmployeeModel:
    ...
    def get_employee_full_details(emp_id):
        ...
        SELECT E.*, O.*, D.Description as ddo_name, DP.description as dept_name, DS.designation as desg_name,
               ...
               CONVERT(varchar, O.dateofappointment, 103) as doa_fmt,
               CONVERT(varchar, O.dateofjoining, 103) as doj_fmt,
               CONVERT(varchar, O.dateOfConfirmation, 103) as doc_fmt,
               CONVERT(varchar, O.dateofretirement, 103) as dor_fmt
        FROM SAL_Employee_Mst E
        LEFT JOIN SAL_EmployeeOther_Details O ON E.pk_empid = O.fk_empid
        LEFT JOIN DDO_Mst D ON E.fk_ddoid = D.pk_ddoid
    ...
    def get_full_lookups():
        return {
            ...
            'quarters': DB.fetch_all("SELECT pk_quarterid as id, quarterdesc as name FROM SAL_Quarter_Mst ORDER BY quarterdesc"),
            'genders': DB.fetch_all("SELECT pk_genderid as id, genderdesc as name FROM SMS_gender_mst ORDER BY genderdesc"),
            'marital_statuses': [
                {'id': '64', 'name': 'Single'}, {'id': '65', 'name': 'Married'}, 
                {'id': '128', 'name': 'Unmarried'}, {'id': '129', 'name': 'Widow'}
            ]
        }
...
class EmployeeDocumentModel:
    ...
    def get_employee_documents(emp_id):
        query = """
            SELECT pk_docid as id, D.*, C.category as category_name,
    ...
    def get_by_id(doc_id):
        return DB.fetch_one("SELECT pk_docid as id, * FROM SAL_EmployeeDocument_Details WHERE pk_docid = ?", [doc_id])

    @staticmethod
    def get_categories():
        return DB.fetch_all("SELECT pk_doccatid as id, category as name FROM SAL_EmployeeDocumentCategory_Mst ORDER BY category")
    ...
    def save(data, user_id):
        ...
        if doc_id:
            sql = """
                ...
                SET fk_doccatid = ?, filename = ISNULL(?, filename), attdesig = ?, 
            ...
            sql = """
                ...
                (fk_empid, fk_doccatid, filename, attdesig, isactive, fk_insUserID, fk_insDateID, fk_updUserID, fk_updDateID)
    ...
    @staticmethod
    def delete(doc_id):
        return DB.execute("DELETE FROM SAL_EmployeeDocument_Details WHERE pk_docid = ?", [doc_id])
...
class ForeignVisitModel:
    ...
    def get_employee_foreign_visits(emp_id):
        return DB.fetch_all("""
            SELECT V.pk_visitid as id, V.*, S.SponsorName as sponsor_name,
            ...
            LEFT JOIN SAL_FundsSponsor_Mst S ON V.FK_FSponsor_Id = S.Pk_FSponsor_Id
            WHERE V.fk_empid = ?
        """, [emp_id])
...
class TrainingModel:
    ...
    def get_employee_trainings(emp_id):
        return DB.fetch_all("""
            SELECT T.pk_trainingid as id, T.*, S.SponsorName as sponsor_name,
            ...
            LEFT JOIN SAL_FundsSponsor_Mst S ON T.FK_FSponsor_Id = S.Pk_FSponsor_Id
            WHERE T.fk_empid = ?
        """, [emp_id])
...
class SARModel:
    ...
    def get_sar_lists(category='T'):
        ...
        base_query = """
            ...
            LEFT JOIN SAL_EmployeeOther_Details O ON E.pk_empid = O.fk_empid
            WHERE S.fk_categoryid = ?
        """
        to_report = DB.fetch_all(base_query + " ORDER BY E.empname", [cat_id])
        to_forward = DB.fetch_all(base_query + " AND S.IsSubmit = 'Y' AND S.FinalApproval = 0 ORDER BY E.empname", [cat_id])
        submitted = DB.fetch_all(base_query + " AND S.FinalApproval = 1 ORDER BY E.empname", [cat_id])
