...
class AuthModel:
    ...
    def authenticate(username, password):
        ...
        if user and user.get('photo'):
            # Clean filename: if it contains a slash, take only the part after the last slash
            photo_raw = str(user['photo'])
            if '/' in photo_raw:
                user['photo'] = photo_raw.split('/')[-1]
            elif ' ' in photo_raw and '.jpg' in photo_raw.lower():
                # Handle cases like "Dr OPS.jpgPhoto..." if they exist without slash
                import re
                match = re.search(r'Photo\d+\.jpg', photo_raw, re.I)
                if match:
                    user['photo'] = match.group()
        return user
