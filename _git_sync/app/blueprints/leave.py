...
from app.models import LeaveModel, NavModel, EmployeeModel, LeaveAssignmentModel, LeaveEncashmentModel, LeaveReportModel, LeaveConfigModel, HolidayModel, WeeklyOffModel, LeaveTypeModel
...
def api_calculate_days():
    ...
    try:
        ...
        # RH Validation: If RH selected but 0 days counted
        nearby_rh = []
        if str(leave_id) == '7' and leave_days == 0:
            try:
                # Find valid RH dates for this location and year
                year = datetime.strptime(f, "%Y-%m-%d").year
                valid_rh = list(LeaveModel._get_rh_dates_for_loc(loc, year))
                valid_rh.sort()
                
                # Get nearest dates (before/after)
                f_date = datetime.strptime(f, "%Y-%m-%d").date()
                nearby = []
                for rh in valid_rh:
                    diff = abs((rh - f_date).days)
                    nearby.append((diff, rh))
                nearby.sort()
                nearby_rh = [rh.strftime("%d/%m/%Y") for diff, rh in nearby[:3]]
            except: pass
...
def leave_adjustment():
    ...
    if selected_leave_id:
        ...
        taken_details = LeaveModel.get_leave_taken_details(selected_leave_id)
...
def approvals():
    ...
    page_h = request.args.get('page_h', 1, type=int)
    page_r = request.args.get('page_r', 1, type=int)
    page_l = request.args.get('page_l', 1, type=int)
    page_u = request.args.get('page_u', 1, type=int)
    per_page = 10
    
    total_p = LeaveModel.get_pending_approvals_count(emp_id)
    pending = LeaveModel.get_pending_approvals(emp_id, page=page_p, per_page=per_page)
    
    history, total_h = LeaveModel.get_approved_recent(emp_id, page=page_h, per_page=per_page)
    rejected, total_r = LeaveModel.get_rejected_requests(emp_id, page=page_r, per_page=per_page)
    
    lower_level, total_l = LeaveModel.get_lower_level_stats(emp_id, page=page_l, per_page=per_page)
    upper_level, total_u = LeaveModel.get_upper_level_stats(emp_id, page=page_u, per_page=per_page)
    
    def get_pagination(page, total, per_page):
        return {
            'page': page,
            'total': total,
            'total_pages': math.ceil(total / per_page) if total else 1,
            'has_prev': page > 1,
            'has_next': page < math.ceil(total / per_page) if total else False,
        }

    pagination_p = get_pagination(page_p, total_p, per_page)
    pagination_h = get_pagination(page_h, total_h, per_page)
    pagination_r = get_pagination(page_r, total_r, per_page)
    pagination_l = get_pagination(page_l, total_l, per_page)
    pagination_u = get_pagination(page_u, total_u, per_page)
    ...
    return render_template('leave/leave_approvals.html', 
                           ...
                           pagination_r=pagination_r,
                           pagination_l=pagination_l,
                           pagination_u=pagination_u,
                           perm=perm,
...
def api_leave_type_master_details(lt_id):
    master = LeaveTypeModel.get_leave_type_by_id(lt_id)
    details = LeaveTypeModel.get_leave_type_details(lt_id)
    return jsonify({
        'master': master,
        'details': details
    })

@leave_bp.route('/api/leave_type/detail/<did>')
def api_leave_type_single_detail(did):
    # Fetch a single row from SAL_Leavetype_Details
    detail = DB.fetch_one("SELECT * FROM SAL_Leavetype_Details WHERE pk_id = ?", [did])
    return jsonify(detail)

@leave_bp.route('/leave_type_master', methods=['GET', 'POST'])
...
def leave_type_master():
    ...
    if request.method == 'POST':
        ...
        try:
            if action == 'SAVE_MASTER':
                data = {
                    'id': request.form.get('pk_leaveid'),
                    'leavetype': request.form.get('leavetype'),
                    'shortdesc': request.form.get('shortdesc'),
                    'fixed_id': request.form.get('fixed_id'),
                    'nature_code': request.form.get('nature_code'),
                    'gender': request.form.get('gender'),
                    'remarks': request.form.get('remarks')
                }
                lt_id = LeaveTypeModel.save_leave_type(data, user_id)
                flash("Leave Type master saved successfully.", "success")
                return redirect(url_for('leave.leave_type_master', edit_id=lt_id))

            elif action == 'SAVE_DETAIL':
                data = {
                    'pk_id': request.form.get('pk_detail_id'),
                    'leave_id': request.form.get('pk_leaveid'),
                    'nature_id': request.form.get('nature_id'),
                    'category': request.form.get('category'),
                    'eligibility': request.form.get('eligibility', 0),
                    'limit_per_instance': request.form.get('limit_per_instance', 0),
                    'max_balance': request.form.get('max_balance', 0),
                    'max_per_year': request.form.get('max_per_year', 0),
                    'hpl': 1 if request.form.get('hpl') else 0,
                    'cf': 1 if request.form.get('cf') else 0,
                    'cf_limit': request.form.get('cf_limit', 0),
                    'cashable': 1 if request.form.get('cashable') else 0,
                    'encashment_after': request.form.get('encashment_after', 0),
                    'off_covered': 1 if request.form.get('off_covered') else 0,
                    'formula_id': request.form.get('formula_id'),
                    'amount': request.form.get('amount', 0),
                    'min_limit': request.form.get('min_limit', 0),
                    'days_before': request.form.get('days_before', 0),
                    'fixed_times': 0, # Add these if needed
                    'total_times': 0
                }
                LeaveTypeModel.save_leave_type_detail(data, user_id)
                flash("Leave Type detail saved successfully.", "success")
                return redirect(url_for('leave.leave_type_master', edit_id=data['leave_id']))

            elif action == 'DELETE_MASTER':
                lt_id = request.form.get('id')
                LeaveTypeModel.delete_leave_type(lt_id)
                flash("Leave Type and its details deleted.", "success")
                return redirect(url_for('leave.leave_type_master'))

            elif action == 'DELETE_DETAIL':
                did = request.form.get('did')
                lt_id = request.form.get('lt_id')
                LeaveTypeModel.delete_leave_type_detail(did)
                flash("Leave Type configuration detail deleted.", "success")
                return redirect(url_for('leave.leave_type_master', edit_id=lt_id))
    ...
    # GET
    leave_types = LeaveTypeModel.get_all_leave_types()
    lookups = LeaveTypeModel.get_lookups()
    
    edit_id = request.args.get('edit_id')
    master = None
    details = []
    if edit_id:
        master = LeaveTypeModel.get_leave_type_by_id(edit_id)
        details = LeaveTypeModel.get_leave_type_details(edit_id)

    return render_template('leave/master_leave_type.html', 
                           leave_types=leave_types, 
                           lookups=lookups,
                           master=master,
                           details=details)
