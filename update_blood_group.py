...
new_query = """    query = '''
        SELECT S.*, 
               ...
               COALESCE(EMP_PG.empname, EMP_UG.empname) as advisor_name,
               RANKMST.Rankname as quota_name,
               ADMCAT.category as adm_category_name,
               BLM.name as blood_group_name
        ...
        LEFT JOIN SMS_Category_Mst ADMCAT ON S.fk_admcategory = ADMCAT.category
        ...
        LEFT JOIN SMS_Advisory_Committee_Dtl ACD ON ACM.pk_adcid = ACD.fk_adcid AND ACD.fk_statusid = 1
        LEFT JOIN SAL_Employee_Mst EMP_PG ON ACD.fk_empid = EMP_PG.pk_empid
        LEFT JOIN sms_AdvisoryStudentApproval ADV ON S.pk_sid = ADV.fk_sturegid AND ADV.fk_stid = 1 AND ADV.ApprovalStatus=1
        LEFT JOIN SAL_Employee_Mst EMP_UG ON ADV.fk_empid = EMP_UG.pk_empid
        LEFT JOIN SMS_blood_mst BLM ON S.B_Group = CAST(BLM.id as nvarchar)
        WHERE S.pk_sid = ?
    '''"""
...
if pattern.search(code):
    ...
    with open('app/blueprints/student_portal/profile.py', 'w', encoding='utf-8') as f:
        f.write(code)
    print('Updated base profile query to pull blood group')
else:
